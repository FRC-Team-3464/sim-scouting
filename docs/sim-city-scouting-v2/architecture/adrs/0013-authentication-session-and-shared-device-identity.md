...
**Approved decisions:** Reuse Firebase identity, Node credential exchange, HttpOnly session cookies, revocation checks, signed CSRF, exact-Origin checks, and the Node-only boundary behind `/api/scouting/v2/auth/*`; bounded versioned public identity/session projection with dynamic scoped grants fetched separately; same-UID reauthentication and explicit account switching; public registration with zero application privileges; Firebase-managed mandatory email verification before membership activation; Firebase-managed password recovery with a thin application request/status surface; `email-verification-requests` and `password-reset-requests` resource naming; retain/discard/cancel handling for unsynchronized work; ordinary current-browser logout with no Scout self-service all-device logout; separate Administrator/operations emergency revocation; six-hour absolute session with 30-minute warning, explicit renewal, and no live-capture idle timeout; server-authoritative credential age with a 15-minute privileged-operation window
...
The session has a six-hour absolute lifetime and warns 30 minutes before expiry. There is no rolling renewal and no automatic inactivity timeout during live capture. Session renewal is explicit reauthentication; a warning never interrupts an active capture. Recent authentication is operation-specific and requires backend-confirmed `authenticatedAt` to be no more than 15 minutes old for role changes, package publication, event overrides, correction or voiding of another Scout's evidence, identifiable exports, audit access, emergency revocation, and destructive cleanup. Server policy, not `FreshSessionRoute`, decides freshness. Ordinary active capture is never freshness-gated.
...
An ordinary role or grant change increments `authorizationVersion` and takes effect through current backend authority without automatically ending the Firebase session. Membership suspension or revocation, lost-device response, or suspected compromise may revoke Firebase sessions. No authorization change deletes, transfers, or silently reattributes unsynchronized evidence; it remains under the original UID.
...
Engineering must test browser cookie behavior, clock skew at the 15-minute freshness and six-hour expiry boundaries, Firebase email-action authorized domains and no-cost quotas, administrator/operations revocation propagation, PWA restart, same-UID enforcement, account switching, and session expiry with unsynchronized work. Self-service all-device logout is out of scope; reconsideration requires a new approved product and architecture decision. No product-owner or principal-architect approval remains open in this ADR.
