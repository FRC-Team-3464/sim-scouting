...
An assignment opens a capture session. The Scout confirms derived context and uses one explicit Start Match action at the observed field cue. A device-local monotonic timer advances season-package-defined phases automatically. The MVP has no routine pause/resume, manual phase navigation, per-observation confidence prompt, or detailed clock-correction workflow. The Scout records configured observations with rapid controls, corrects mistakes without destroying audit history, reviews the result, flags timing or incompleteness concerns with an optional note when necessary, and queues it for synchronization. An interrupted capture restores from locally persisted timing anchors and identifies that restoration without pretending it is authoritative field time.

For high-throughput numeric observations, the familiar v1 concept of one visible running total with rapid positive and negative preset adjustments is the approved MVP interaction baseline. V2 must redesign its visual hierarchy, feedback, accessibility, responsiveness, and error recovery rather than reproduce the v1 screen. Preset deltas, labels, limits, and phase applicability are season-package configuration, not permanent UI constants.

The multi-delta counter is not universal. Every Match and Pit observation independently selects an allow-listed controlled component such as an action, categorical append action, counter, integer range, explicit binary choice, categorical choice, state machine, checklist, anchored rating, measurement, categorized spatial action, or note. Repeated mutually exclusive outcomes such as made/missed use a categorical append action when validation selects that method: every activation appends evidence and never replaces a prior action. A spatial action records both its configured action option and location. Timer, stopwatch, and cycle totals are not raw component payloads; duration and cycle metrics derive from timestamped actions or state transitions. The Season Package contract defines component behavior and payloads; scouting-method validation selects the least burdensome appropriate kind and configuration for each season field.

Multi-delta preset sizes, made/missed attempts, cycle events, rate intervals, quantity ranges, estimate-confidence semantics, spatial granularity, and rating anchors remain configurable candidates. Their defaults must be chosen through [scouting-method validation](../validation/scouting-method-validation.md), not assumed by architecture or UI implementation. Advanced timer controls and granular confidence prompting are post-MVP candidates rather than unresolved MVP requirements.
...
The MVP must be complete and useful without photos. Robot photos are an optional, deferred enhancement for identification and visual context; they are never required evidence and must not block creating, reviewing, syncing, or using a structured pit contribution. MVP provides no upload, dedicated external photo/album link, preview, or external-media workflow. No blob-storage provider, paid billing plan, upload endpoint, retention policy, or photo quota is required for MVP.

If photos are later approved, their rollout requires an explicit storage/cost decision, measured event usage, retention limits, accessibility descriptions, metadata stripping, and graceful operation when uploads are disabled. The data model declares optional `photoIds` only as a future compatibility point; the field must be absent from MVP requests and canonical revisions and establishes no current photo identifier or resource.
